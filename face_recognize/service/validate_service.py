...
def detect(filename, cascade_file="../static/model/haarcascade/haarcascade_frontalface_alt.xml"):
...
def validate(image1, image2):
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, '../static/model/deep_id/25000.ckpt')
        h1 = sess.run(validate, {input: pic1arr})
        h2 = sess.run(validate, {input: pic2arr})
    with open('../static/model/joint_bayesian/A.pkl', 'rb') as f:
        A = pickle.load(f)
    with open('../static/model/joint_bayesian/G.pkl', 'rb') as f:
        G = pickle.load(f)
